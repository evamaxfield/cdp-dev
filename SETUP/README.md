...
1.  Create the GitHub repository for this deployment to live in.
    ...
    -   Set the repo owner to: **JacksonMaxfield**
...
1.  Create (or re-use) a
    [Google Cloud billing account](https://console.cloud.google.com/billing/linkedaccount?project=cdp-jackson-dev-077)
    and attach it to the newly created project (cdp-jackson-dev-077).
...
1.  Generate a Google Service Account JSON Key for your Google Cloud Project.
    ...
    add a file called `cdp-jackson-dev-077.json` to it
    (i.e. `.keys/cdp-jackson-dev-077)`. This file will be used later on.
...
1.  Attach the Pulumi Access Token and the
    ...
    1. Pulumi Access Token -- Create a [new secret](https://github.com/JacksonMaxfield/cdp-dev/settings/secrets/actions/new)

    -   Set the name to: **PULUMI_ACCESS_TOKEN**
    -   Set the value to: The token you kept from step #2
    -   Click "Add secret"

    2. Google Service Account JSON -- Create a [new secret](https://github.com/JacksonMaxfield/cdp-dev/settings/secrets/actions/new)

    -   Set the name to: **GOOGLE_CREDENTIALS**
    -   Set the value to: the contents of the file `.keys/cdp-jackson-dev-077.json`
    -   Click "Add secret"
...
1.  Initialize and push the local repository to GitHub.
    ...
    ```bash
    git remote add origin https://github.com/JacksonMaxfield/cdp-dev.git
    ```

    Or (with SSH):

    ```bash
    git remote add origin git@github.com:JacksonMaxfield/cdp-dev.git
    ```
...
1.  Once the
    ["Web App" GitHub Action Successfully Complete](https://github.com/JacksonMaxfield/cdp-dev/actions?query=workflow%3A%22Web+App%22)
    configure GitHub Pages.

    Go to your repository's [GitHub Pages Configuration](https://github.com/JacksonMaxfield/cdp-dev/settings/pages)
...
1.  Once the
    ["Infrastructure" GitHub Action Successfully Completes](https://github.com/JacksonMaxfield/cdp-dev/actions?query=workflow%3A%22Infrastructure%22)
...
1.  Once the
    ...
    [Direct Link to Enable](https://console.cloud.google.com/apis/api/speech.googleapis.com/data_logging?project=cdp-jackson-dev-077)
...
1.  Once the
    ...
    -   Navigate to [Firebase Console](https://console.firebase.google.com),
        login to the Google Account you used during step #2, select the `cdp-jackson-dev-077` Firebase project
...
**If all steps complete successful your web application will be viewable at: https://JacksonMaxfield.github.io/cdp-dev**
